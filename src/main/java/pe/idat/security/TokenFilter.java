...
public class TokenFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (tokenHeader != null && tokenHeader.startsWith("Bearer")) {		// cuando se trabaja con una autorizacion, siempre se tiene que trabajar con BEARER (eso aparecee en el postman)
		...
		if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
			...
			if (util.validateToken(token, user)) {			// Si este USER existe en ese TOKEN
